...
class DatepickerSettings:
    def __init__(self, initial_date: date = datetime.now(tz=None).date(),
                 initial_view: str = 'day',
                 views: Dict[str, Dict[str, Union[str, List[str], bool, None]]] = None,
                 labels: Dict[str, str] = None,
                 custom_actions: Optional[List[Type[DatepickerCustomAction]]] = None):
    ...
    def initial_views_validate(self, v):
        ...
        if 'day' in v:
            ...
            self.tzinfo = views['day']['tzinfo']
            self.initial_date = datetime.now(tz=ZoneInfo(self.tzinfo)).date()
